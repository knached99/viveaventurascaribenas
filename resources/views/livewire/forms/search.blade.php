<div class="w-full max-w-lg relative"> <!-- Make this relative to contain the absolute child -->
    ...
    @if (!empty($searchResults))
        ...
        <ul class="list-group m-0 p-0 bg-white shadow-lg border border-gray-200 rounded-lg mt-2 absolute w-full max-h-[400px] overflow-y-auto z-40">
           ...
           @foreach ($searchResults as $result)
            ...
            $tripImages = isset($result['trip']) ? json_decode($result['trip']['tripPhoto'], true) : json_decode($result['tripPhoto'], true);
            $imageSrc = $tripImages[0] ?? null;

            // Set location for alt text
            $location = $result['type'] === 'trip' ? $result['tripLocation'] : $result['trip']['tripLocation'] ?? 'Undefined Location';
             @endphp
        ...
        @if ($result['type'] === 'trip')
            <a href="{{ route('admin.trip', ['tripID' => $result['tripID']]) }}" class="text-decoration-none">
                <li class="list-group-item p-2 border-b hover:bg-gray-100 cursor-pointer">
                    <img src="{{ $imageSrc }}" alt="{{ $location }}"class="w-16 h-16 rounded-lg object-cover border border-gray-200 shadow-md rounded" />
        ...
        @elseif($result['type'] === 'booking')
            <a href="{{ route('admin.booking', ['bookingID' => $result['bookingID']]) }}" class="text-decoration-none">
                <li class="list-group-item p-4 border-b hover:bg-gray-100 cursor-pointer flex items-center">
                    ...
                    <div class="ml-4">
                        <h5 class="mb-1 font-semibold text-gray-800">{{ $result['name'] ?? 'Unnamed Booking' }}</h5>
                        <p class="mb-1 text-gray-500">{{ $result['email'] ?? 'No Email' }} | {{ $result['phone_number'] ?? 'No Phone Number' }}</p>
        ...
        @elseif($result['type'] === 'reservation')
            <a href="{{ route('admin.reservation', ['reservationID' => $result['reservationID']]) }}" class="text-decoration-none">
                <li class="list-group-item p-4 border-b hover:bg-gray-100 cursor-pointer flex items-center">
                    <div class="flex-shrink-0">
                        <img src="{{ $imageSrc }}" alt="{{ $location }}" class="w-16 h-16 rounded-lg object-cover border border-gray-200 shadow-md rounded" />
                    </div>
                    <div class="ml-4">
                        <h5 class="mb-1 font-semibold text-gray-800">{{ $result['name'] ?? 'Unnamed Reservation' }}</h5>
                        <p class="mb-1 text-gray-500">{{ $result['email'] ?? 'No Email' }} | {{ $result['phone_number'] ?? 'No Phone Number' }}</p>
                        <p class="mb-0 text-gray-600">
        ...
        @elseif($result['type'] === 'testimonial')
            <a href="{{ route('admin.testimonial', ['testimonialID' => $result['testimonialID']]) }}" class="text-decoration-none">
                <li class="list-group-item p-4 border-b hover:bg-gray-100 cursor-pointer flex items-center">
                    ...
                    <div class="ml-4">
                        <h5 class="mb-1 font-semibold text-gray-800">{{ $result['name'] ?? 'Unnamed Reservation' }}</h5>
                        <p class="mb-1 text-gray-500">{{ $result['name'] ?? 'Anonymous' }} | {{ $result['email'] ?? 'No Email' }}</p>
                        <p class="mb-0 text-gray-600">
