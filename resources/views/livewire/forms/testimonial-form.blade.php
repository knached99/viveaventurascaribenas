...
<div>
 ...
 <h2 class="mb-4">Share Your Travel Experience with Us!</h2>
    ...
    <form wire:submit.prevent="submitTestimonialForm" class="row g-3">
    <x-honeypot livewire-model="extraFields" />
        ...
        <div class="col-12">
            <div class="form-group">
                <input type="email" wire:model="email" class="form-control {{$errors->has('email') ? 'border border-danger': ''}}" placeholder="Your Email (required for follow-up)">
                @error('email')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
