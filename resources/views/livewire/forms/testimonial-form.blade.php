...
<form class="row g-3" wire:submit.prevent="submitTestimonialForm">
    ...
    <div class="col-12">
        <div class="form-group">
            ...
            <input wire:model="trip_date" type="month" max="{{ date('Y-m') }}" value="{{ date('Y-m') }}"
    ...
    <div class="col-12">
        <div class="form-group">
            ...
            <div class="rating">
                @for ($i = 5; $i >= 1; $i--)
                    <input wire:model="trip_rating" type="radio" name="rating" value="{{ $i }}" id="{{ $i }}">
                    <label for="{{ $i }}">☆</label>
                @endfor
            </div>
            @error('trip_rating')
    ...
    <div class="col-12">
        <div class="form-group">
            <textarea wire:model="testimonial" class="form-control {{ $errors->has('testimonial') ? 'border border-danger' : '' }}" rows="7"
                placeholder="Tell us about your experience (What made your trip special?)"></textarea>
            @error('testimonial')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
