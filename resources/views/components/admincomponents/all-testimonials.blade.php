...
<table class="table">
    ...
    @elseif(session('testimonial_delete_error'))
        <p class="text-danger m-3">{{ session('testimonial_delete_error') }}</p>
    @endif
    <thead>
        <tr>
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex">
                        @for ($i = 1; $i <= 5; $i++)
                            <i
                                class="bx bxs-star {{ $i <= $testimonial->trip_rating ? 'text-warning' : 'text-secondary' }}"></i>
                        @endfor
                    </div>
                </td>
                ...
                <td>
                    <a href="{{ route('admin.testimonial', ['testimonialID' => $testimonial->testimonialID]) }}"
