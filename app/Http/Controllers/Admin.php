...
class Admin extends Controller
{
    ...
    public function dashboardPage(){
        ...
        */
        ...
        $bookings = BookingModel::with('trip')->select('bookingID', 'square_payment_id', 'square_catalog_object_id', 'tripID', 'created_at')->get();

        $reservations = Reservations::with('trip')->select('reservationID', 'square_catalog_object_id', 'tripID', 'name', 'email', 'phone_number', 'address_line_1', 'address_line_2', 'city', 'state', 'zip_code')->get();

        // optimizing square api calls 
        
        $allCatalogObjectIds = $bookings->pluck('square_catalog_object_id')->merge($reservations->pluck('square_catalog_object_id'))->unique()->values()->toArray();
        $productMap = collect($squareProducts)->mapWithKeys(fn($product) => [$product->getId() => $product->getItemData()->getName()]);
        
        // most popular booking 
        $mostPopularBookings = BookingModel::select('square_catalog_object_id', DB::raw('COUNT(*) as booking_count'))
        ->groupBy('square_catalog_object_id')->having('booking_count', '>', 2)->orderByDesc('booking_count')->first();

         $mostPopularReservations = Reservations::select('square_catalog_object_id', DB::raw('COUNT(*) as reservation_count'))
        ->groupBy('square_catalog_object_id')->having('reservation_count', '>', 2)->orderByDesc('reservation_count')->first();
    
        $mostPopularTripName = $mostPopularTripPhoto = null;

        if($mostPopularBookings){

            $trip = TripsModel::where('square_catalog_object_id', $mostPopularBookings->square_catalog_object_id)->first();
